...
class Uts:
    ...
    def refresh_player_status(client_id: int) -> bool:
        ...
        new_status = {
            'admin': CommandFunctions.user_is_admin(client_id),
            'captain': False,
            'club_member': False,
            'tag': None,
            'timestamp': now
        ...
        # إذا كان أدمن، نوقف التحقق (لا حاجة لبقية الفحوصات)
        if new_status['admin']:
            if old.get('admin') != new_status['admin']:
                Uts.player_status_cache[client_id] = new_status
                return True
            return False

        # ليس أدمن، نكمل التحقق
        ...
        if account_id and account_id.startswith('pb-') and account_id in Uts.pdata:
            ...
            # التحقق من كابتن النادي
            if 'club' in player_data:
                club_info = player_data['club']
                new_status['club_member'] = True
                if club_info.get('role') == 'captain':
                    new_status['captain'] = True

            # التحقق من وجود تاج مخصص
            if 'Tag' in player_data:
                new_status['tag'] = player_data['Tag'].get('text')

        # مقارنة مع الحالة القديمة
        changed = (
            old.get('admin') != new_status['admin'] or
            old.get('captain') != new_status['captain'] or
            old.get('club_member') != new_status['club_member'] or
            old.get('tag') != new_status['tag']
        )
        if changed:
            Uts.player_status_cache[client_id] = new_status
        return changed
    ...
    def check_player_changes():
        """مقارنة اللاعبين الحاليين مع السابقين وإزالة التيجان والكاش للمغادرين"""
        current_players = {}
        try:
            activity = bs.get_foreground_host_activity()
            if activity is not None:
    ...
    def update_usernames() -> None:
        """
        تحديث الأسماء و pb-id فقط (بدون تطبيق التيجان)
        """
        try:
            # التحديث من roster
            for r in roster():
        ...
        # بعد التحديث، نتحقق من تغيرات اللاعبين (ولكن دون تطبيق التيجان هنا)
...
class Commands:
    ...
    def find_target_data(self, target):
        """
        البحث عن بيانات اللاعب باستخدام:
        - Client ID (رقم)
        - PB-ID (يبدأ بـ pb-)
        - الاسم الكامل (من usernames أو roster أو pdata)
        - كلمة 'all' لاستهداف جميع اللاعبين
        """
        target = str(target).strip()
        if not target:
            return None

        # 1. البحث باستخدام Client ID (رقم صحيح)
        try:
            c_id = int(target)
            # التحقق من وجوده في usernames أولاً
            if c_id in Uts.usernames:
                name = Uts.usernames[c_id]
                account_id = Uts.get_reliable_pb_id(c_id)
                return {
                    'client_id': c_id,
                    'account_id': account_id,
                    'name': name,
            ...
            # إذا لم نجده في usernames، نبحث في roster
            for r in roster():
                if r.get('client_id') == c_id:
                    account_id = r.get('account_id')
                    name = r.get('display_string', f"Player {c_id}")
                    # تخزين PB-ID في userpbs للاستخدام المستقبلي
                    if account_id and account_id.startswith('pb-') and c_id is not None:
            ...
            pass  # ليس Client ID رقمي

        # 2. البحث باستخدام PB-ID (يبدأ بـ pb-)
        if target.startswith('pb-'):
            # نبحث عن client_id لهذا الحساب (إذا كان متصلاً حالياً)
            for c_id, acc in Uts.userpbs.items():
                if acc == target:
                    name = Uts.usernames.get(c_id, target)
                    return {
                        'client_id': c_id,
                        'account_id': target,
                        'name': name,
            ...
            # إذا لم يكن متصلاً، نعيد بيانات جزئية
            return {
                'client_id': None,
                'account_id': target,
                'name': target,
        ...
        # 3. البحث بالاسم في usernames (قاموس الأسماء الحالية)
        target_lower = target.lower()
        for c_id, name in list(Uts.usernames.items()):
        ...
        # 4. البحث بالاسم في roster (قائمة اللاعبين الحاليين)
        for r in roster():
            ...
            # البحث في قائمة players داخل كل إدخال
            for player in r.get('players', []):
                player_name = player.get('name_full', '')
                if player_name.lower() == target_lower:
                    client_id = r.get('client_id')
                    account_id = r.get('account_id')
                    if account_id and account_id.startswith('pb-') and client_id is not None:
                        Uts.userpbs[client_id] = account_id
        ...
        # 5. البحث في pdata عبر قائمة Accounts (للاعبين غير المتصلين حالياً)
        for acc_id, acc_data in Uts.pdata.items():
            accounts_list = acc_data.get('Accounts', [])
            for stored_name in accounts_list:
                if stored_name.lower() == target_lower:
                    # البحث عن client_id لهذا الحساب إذا كان متصلاً
                    for c_id, pb in Uts.userpbs.items():
                        if pb == acc_id:
                            return {
                                'client_id': c_id,
                                'account_id': acc_id,
        ...
        # 6. كلمة "all" لاستهداف جميع اللاعبين
        if target_lower in ['all', 'الكل', 'كل', 'جميع']:
            return {
                'client_id': -999,
                'account_id': 'all',
                'name': 'All Players',
            ...
            if roster_data:
                for r in roster_data:
                    try:
                        ...
                        tag_text = ''
                        if account_id and account_id in Uts.pdata:
                            tag_data = Uts.pdata[account_id].get('Tag', {})
    ...
    def process_photo_command(self, msg: str, client_id: int):
        ...
        try:
            ...
            # إذا لم تكن القائمة موجودة، ننشئها
            if not hasattr(self, '_photo_sessions'):
                self._photo_sessions = []

            class PhotoSession:
                def __init__(self, act, clr):
                    self.activity = act
                    self.color = clr
            ...
            session = PhotoSession(activity, color)
            self._photo_sessions.append(session)

            self.clientmessage(f"📸 New photo session created with color ({r},{g},{b}) - total: {len(self._photo_sessions)}", color=(0,1,0))
    ...
    def process_photoclear_command(self, client_id: int):
        ...
        try:
            if hasattr(self, '_photo_sessions') and self._photo_sessions:
                count = len(self._photo_sessions)
                for session in self._photo_sessions:
                    session.cleanup()
                self._photo_sessions.clear()
